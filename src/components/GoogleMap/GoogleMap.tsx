    ...
    public componentWillReceiveProps(newProps: IGoogleMapProps): void {
        if(newProps.locationCoords && newProps.locationCoords.length > 1) {
            ...
            if(newProps.findRoute) {
                 ...
                 */
                fetch('/api/routes/find-near?latOrig='+markerCoords[0].lat()+'&lngOrig='+markerCoords[0].lng()+'&lngDest='+markerCoords[markerCoords.length-1].lat()+'&latDest='+markerCoords[markerCoords.length-1].lng()).then((response: any) => {
                    ...
                    responseJson.directions.forEach((response: any)=>{
                        ...
                        routeRequest = {
                            ...
                            waypoints: response.stops.map((stop: any)=>new google.maps.LatLng(stop.geometry.coordinates[0], stop.geometry.coordinates[1])),
                    ...
                    this.state.activeMarkers.forEach((marker)=> {
                        marker.setMap(null);
                    });
